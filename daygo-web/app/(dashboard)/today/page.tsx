...
export default function TodayPage() {
  ...
  // Sync local big wins state with query result
  useEffect(() => {
    if (dailyNote?.big_wins) {
      const wins = dailyNote.big_wins as string[]
      setLocalBigWins([wins[0] || '', wins[1] || '', wins[2] || ''])
    } else {
      setLocalBigWins(['', '', ''])
    }
  }, [dailyNote?.big_wins])
  ...
  const saveBigWinsMutation = useMutation({
    mutationFn: (wins: string[]) => dailyNotesService.saveBigWins(user!.id, dateStr, wins),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['daily-note', user?.id, dateStr] })
    },
  })
    ...
    <div {...swipeHandlers} className="max-w-lg mx-auto px-5 py-8 pb-32 min-h-screen bg-gradient-to-b from-bevel-bg to-white dark:from-slate-900 dark:to-slate-950 overflow-x-hidden">
      ...
      {isLoading ? (
        ...
        >
          ...
          {/* 3 Big Wins */}
          <section className="bg-gradient-to-br from-amber-50/50 to-yellow-50/50 dark:from-amber-900/10 dark:to-yellow-900/10 rounded-2xl p-4 -mx-4">
            <div className="flex items-center gap-2 mb-4">
              <Trophy className="w-5 h-5 text-amber-500" />
              <h2 className="section-header text-bevel-text-secondary dark:text-slate-400">
                3 Big Wins for Today
              </h2>
            </div>
            <div className="space-y-3">
              {[0, 1, 2].map((i) => (
                <div key={i} className="flex items-center gap-3">
                  ...
                  <input
                    type="text"
                    value={localBigWins[i] || ''}
                    onChange={(e) => {
                      const updated = [...localBigWins]
                      updated[i] = e.target.value
                      setLocalBigWins(updated)
                    }}
                    onBlur={() => saveBigWinsMutation.mutate(localBigWins)}
                    onKeyDown={(e) => {
                      if (e.key === 'Enter') {
                        (e.target as HTMLInputElement).blur()
                      }
                    }}
                    placeholder={i === 0 ? 'What\'s your #1 win today?' : i === 1 ? 'Second big win...' : 'Third big win...'}
                    className="flex-1 px-3 py-2.5 rounded-xl border border-amber-200/50 dark:border-amber-500/20 bg-white dark:bg-slate-800/50 text-sm text-bevel-text dark:text-white placeholder-gray-400 dark:placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-amber-400/50 transition-all"
                  />
                </div>
              ))}
            </div>
